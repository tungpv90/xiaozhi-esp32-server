# Hướng dẫn dùng LLM riêng của bạn
...
- Có endpoint `/v1/chat/completions`
- Hỗ trợ `messages`
- Tốt hơn nếu hỗ trợ `stream`
- Nếu muốn function call, endpoint phải hỗ trợ `tools`
...
- `base_url`: địa chỉ API của model, nếu chưa có `/v1` thì nên ghi rõ
...
Provider đã có sẵn: `type: ollama`
...
- nếu muốn tool call, trả `tool_calls` đúng format OpenAI
...
- `AliBL` và `FastGPT` hỗ trợ hạn chế hơn tùy backend
- `Home Assistant` không dùng function call kiểu Xiaozhi AI, nó tự xử lý theo luồng riêng

Nếu model của bạn không hỗ trợ tool calling:

- server vẫn chat bình thường
...
Prompt hệ thống sẽ được build lại khi có session mới. Sau khi đổi config:
...
- nếu cần tool call, bảo đảm backend hỗ trợ `tools`
